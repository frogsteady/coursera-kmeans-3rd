...
class KMeans {
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.forall(pred => newMeans(oldMeans.indexOf(pred)).squareDistance(pred) <= eta)
  }

  @tailrec
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    val newMeans = Option(classify(points, means)).map(update(_, means)).getOrElse(GenSeq())
